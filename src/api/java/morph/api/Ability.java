...
public abstract class Ability {
    ...
    /**
     * Ability parent field. Will be null for instances used in registration. Ability is then cloned and parent assigned
     * later on.
     */
    ...
    /**
     * Called when the ability is finally removed when the parent demorphs or morphs into a state that does not have
     * this ability type.
     ...
     * As previously stated before the ability instance used during registration is a base so it needs to be cloned for
     * use with parents.
     */
    public abstract Ability clone();

    /**
     * Return true for this if you need an inactive copy of this morph in-between morph states (abilities of the next
     * morph are only swapped over when morph is complete)
     ...
     * Mainly used for synching Abilities between the client-server for mod mobs which do not use the API to add
     * abilities.
     ...
     * Adds on to the previous ability list, so this allows you to add abilities to Entity classes which already have
     * abilities mapped.
     * However, only one ability of the same type is allowed for each entity. This method will overwrite abilities of
     * the same type that were already mapped.
     ...
     * Check out AbilityHandler to see each Ability type and the parse function in their respective classes for the
     * arguments.
     * 
     * @return
     */
