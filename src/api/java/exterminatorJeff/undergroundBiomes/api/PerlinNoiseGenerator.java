...
public class PerlinNoiseGenerator extends NoiseGenerator {
    ...
    protected PerlinNoiseGenerator() {
        ...
        for (int i = 0; i < 512; i++) this.perm[i] = p[(i & 0xFF)];
    }
    ...
    public double noise(double x, double y, double z) {
        ...
        return lerp(
            ...
            lerp(
                fY,
                lerp(fX, grad(this.perm[AA], x, y, z), grad(this.perm[BA], x - 1.0D, y, z)),
                lerp(fX, grad(this.perm[AB], x, y - 1.0D, z), grad(this.perm[BB], x - 1.0D, y - 1.0D, z))),
            lerp(
                fY,
                lerp(fX, grad(this.perm[(AA + 1)], x, y, z - 1.0D), grad(this.perm[(BA + 1)], x - 1.0D, y, z - 1.0D)),
                lerp(
                    fX,
