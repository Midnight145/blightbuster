...
public class ScanBlocks extends CommandBase {
	...
	public class CommandBlockScanner {
		...
		public String toBlockString(ItemStack stack) {
			...
			if (stack.getItemDamage() > 0) { result.append(':').append(stack.getItemDamage()); }
			result.append('>');
		...
		private void writeValues() {
			try {
				...
				for (String key : blockData.keySet()) {
					...
					writer.write(
							key + "," + Integer.toString(count) + "," + Double.toString(percent) + System.getProperty("line.separator"));
				}
