...
public class C2KModularToolsPage extends OreDiscoveryPage {
	...
	public void readPageFromXML(Element element) {
		...
		if (nodes != null && nodes.item(0) != null) { requires = nodes.item(0).getTextContent(); }
		...
		for (int i = 0; i < 2; i++) {
			...
			if (nodes != null && nodes.item(0) != null && nodes.item(0).getTextContent() != null) // who knows what could go wrong? :/
			{
		...
		if (material == null) { System.err.println("Warning! " + title + " could not be found as a material!"); }
	}
	...
	private void drawNormal(int localWidth, int localHeight) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, icons[0], localWidth + 150,
				localHeight + 75);
		...
		if (material != null) {
			manual.fonts.drawString(harvestLevel + ": " + material.harvestLevel() + " ("
					+ HarvestLevels.getHarvestLevelName(material.harvestLevel()) + "\u00a70)", localWidth, localHeight + 60, 0);
			...
			manual.fonts.drawString(xpRequired + ": " + (XPAdjustmentMap.get(material.materialName) * 100f) + "% " + normal, localWidth,
					localHeight + 110, 0);
	...
	private void drawLocked(int localWidth, int localHeight) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, icons[1], localWidth + 75,
				localHeight + 75);
