...
public class C2KModularToolsPage extends OreDiscoveryPage {
	...
	public void readPageFromXML(Element element) {
		...
		if (nodes != null && nodes.item(0) != null) {
			requires = nodes.item(0).getTextContent();
		}
		...
		for (int i = 0; i < 2; i++) {
			...
			if (nodes != null && nodes.item(0) != null && nodes.item(0).getTextContent() != null) // who knows what
																									 // could go wrong?
																									 // :/
			{
	...
	private void drawNormal(int localWidth, int localHeight) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, icons[0],
				localWidth + 150, localHeight + 75);
		...
		if (material != null) {
			manual.fonts.drawString(
					harvestLevel + ": " + material.harvestLevel() + " ("
							+ HarvestLevels.getHarvestLevelName(material.harvestLevel()) + "\u00a70)",
					localWidth, localHeight + 60, 0);
			manual.fonts.drawString(durability + ": " + material.durability(), localWidth, localHeight + 70, 0);
			manual.fonts.drawString(handleModifier + ": " + material.handleDurability() + "x", localWidth,
					localHeight + 80, 0);
			...
			String heart = (attack == 2 ? " " + heart_ : " " + hearts); // What attention to detail! Thanks,
																		 // Slimeknights!
			if (attack % 2 == 0) {
				manual.fonts.drawString(baseAttack + ": " + material.attack() / 2 + heart, localWidth,
						localHeight + 100, 0);
			}
			else {
				manual.fonts.drawString(baseAttack + ": " + material.attack() / 2f + heart, localWidth,
						localHeight + 100, 0);
			}
			manual.fonts.drawString(
					xpRequired + ": " + (XPAdjustmentMap.get(material.materialName) * 100f) + "% " + normal, localWidth,
					localHeight + 110, 0);
	...
	private void drawLocked(int localWidth, int localHeight) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, icons[1],
				localWidth + 75, localHeight + 75);
