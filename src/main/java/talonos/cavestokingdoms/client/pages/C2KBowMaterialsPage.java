...
public class C2KBowMaterialsPage extends OreDiscoveryPage {
		...
		try {
			for (int i = 0; i < 2; i++) {
				...
				if (nodes != null && nodes.item(i) != null) {
					matNames[i] = nodes.item(i).getTextContent();
				}
				nodes = element.getElementsByTagName("text");
				if (nodes != null && nodes.item(i) != null) {
					description[i] = nodes.item(i).getTextContent();
				}
				
				nodes = element.getElementsByTagName("requires");
				if (nodes != null && nodes.item(i) != null) {
					requires[i] = nodes.item(i).getTextContent();
				}
	...
	private void renderUnlocked(int localWidth, int localHeight, int i) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, icons[i],
				localWidth + 150, localHeight + 4);
		...
		if (materials[i] != null) {
			...
			if (bowsOn) {
				...
				manual.fonts.drawString(flightSpeed + ": " + bowMaterials[i].flightSpeedMax, localWidth,
						localHeight + 40, 0);
			}
			manual.fonts.drawString(xpRequired + ": " + (XPAdjustmentMap.get(materials[i].materialName) * 100f) + "% ",
					localWidth, localHeight + 50, 0);
			...
			if (arrowsOn) {
				...
				manual.fonts.drawString(breakChance + ": " + (arrowMaterials[i].breakChance * 15f) + "% ",
						localWidth + 85, localHeight + 30, 0);
			}
			...
			String totalString = baseAttack + ": "
					+ (attack % 2 == 0 ? materials[i].attack() / 2 : materials[i].attack() / 2f) + heart;
	...
	private void drawLocked(int localWidth, int localHeight, int i) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, requiredIcon[i],
				localWidth + 75, localHeight + 38);
