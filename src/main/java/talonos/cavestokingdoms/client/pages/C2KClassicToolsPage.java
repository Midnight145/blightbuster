...
public class C2KClassicToolsPage extends OreDiscoveryPage {
	...
	public void readPageFromXML(Element element) {
		for (int i = 0; i < 2; i++) {
			...
			if (nodes != null) { title[i] = nodes.item(i).getTextContent(); }
			nodes = element.getElementsByTagName("text");
			if (nodes != null) { description[i] = nodes.item(i).getTextContent(); }

			nodes = element.getElementsByTagName("requires");
			if (nodes != null && nodes.item(0) != null) { requires[i] = nodes.item(i).getTextContent(); }
			...
			if (nodes != null && nodes.item(i) != null) {
				...
				for (int j = 0; j < 5; j++) {
					...
					if (iconItem != null) { icons[j][i] = new ItemStack(GameRegistry.findItem(mod, itemName), 1, meta); }
				}
			}
			...
			if (nodes != null && nodes.item(i) != null && nodes.item(i).getTextContent() != null) // who knows what could go wrong? :/
			{
	...
	private void drawNormal(int localWidth, int localHeight, int i) {
		...
		if (title[i] != null) { manual.fonts.drawString("\u00a7n" + title[i], localWidth + 70, localHeight + 4, 0); }
		...
		for (int j = 0; j < 5; j++) {
			if (icons[j][i] != null) {
				manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, icons[j][i], localWidth + 22 * j,
						localHeight + 28);
				if (icons[j][i].getItem() instanceof ItemPickaxe) { pickItem = (ItemPickaxe) icons[j][i].getItem(); }
			}
		}
