...
public class OreDiscoveryRegistry {
	...
	public OreDiscoveryRegistry() {
		...
		registerDiscovery(GameRegistry.findItem("appliedenergistics2", "item.ItemMultiMaterial"), 1,
	...
	public void onPlayerClone(net.minecraftforge.event.entity.player.PlayerEvent.Clone event) {
		if (event.wasDeath) {
			if (event.original.getEntityData().hasKey("cavesToKingdomsOreDiscoveries")) {
				event.entity.getEntityData().setTag("cavesToKingdomsOreDiscoveries",
						event.original.getEntityData().getTag("cavesToKingdomsOreDiscoveries"));
			}
		}
	}
