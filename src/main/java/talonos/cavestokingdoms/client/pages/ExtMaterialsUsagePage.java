...
public class ExtMaterialsUsagePage extends BookPage {
				...
				if (nodes.item(0).getNodeType() == Node.ELEMENT_NODE) {
					...
					if (toolNodes != null) {
						for (int x = 0; x < toolNodes.getLength(); x++) {
							...
							if (iconItem != null) { tools.add(new ItemStack(GameRegistry.findItem(mod, itemName))); }
						}
					}
				}
			}
				...
				if (nodes.item(0).getNodeType() == Node.ELEMENT_NODE) {
					...
					if (armorNodes != null) {
						for (int x = 0; x < armorNodes.getLength(); x++) {
							...
							String itemName = total.substring(total.indexOf(':') + 1);
							Item iconItem = GameRegistry.findItem(mod, itemName);
							if (iconItem != null) { armors.add(new ItemStack(GameRegistry.findItem(mod, itemName))); }
						}
					}
				}
			}
	...
	public void renderContentLayer(int localWidth, int localHeight, boolean isTranslatable) {
		...
		for (int tool = 0; tool < tools.size(); tool++) {
			manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, tools.get(tool),
					localWidth + 10 + (tool * 22), localHeight + 25);
		}

		for (int armor = 0; armor < armors.size(); armor++) {
			manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, armors.get(armor), localWidth + 6,
		...
		for (int armor = 0; armor < armors.size(); armor++) {
			...
			if (armorStack.getItem() instanceof ItemArmor) {
				...
				manual.fonts.drawString("Durability: " + maxDurability + "   Armor Points: " + protection, localWidth + 24,
						localHeight + 87 + (armor * 18), 0);
			}
		}
