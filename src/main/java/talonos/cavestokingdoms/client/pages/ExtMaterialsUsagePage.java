...
public class ExtMaterialsUsagePage extends BookPage {
	...
	public void renderContentLayer(int localWidth, int localHeight, boolean isTranslatable) {
		...
		for (int armor = 0; armor < armors.size(); armor++) {
			manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, armors.get(armor),
					localWidth + 6, localHeight + 83 + (armor * 18));
		}
		...
		for (int armor = 0; armor < armors.size(); armor++) {
			...
			if (armorStack.getItem() instanceof ItemArmor) {
				...
				manual.fonts.drawString("Durability: " + maxDurability + "   Armor Points: " + protection,
						localWidth + 24, localHeight + 87 + (armor * 18), 0);
			}
		}
