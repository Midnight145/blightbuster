...
public class C2KMiningPage extends OreDiscoveryPage {
	...
	private void drawNormal(int localWidth, int localHeight) {
		...
		manual.renderitem.renderItemAndEffectIntoGUI(manual.fonts, manual.getMC().renderEngine, ore, localWidth + 4,
				localHeight + 4);
		...
		if (this.minesAsLevel != -1) {
			manual.fonts.drawString(
					minesAsLevel + ": " + this.minesAsLevel + " ("
		...
		manual.fonts.drawSplitString("\u00a7l" + biome + ": \u00a7r" + whereFound, localWidth + 0, localHeight + 60,
				106, 0);
		manual.fonts.drawSplitString(
	...
	private void drawLocked(int localWidth, int localHeight) {
		...
		manual.fonts.drawString("\u00a7n" + entryOn + " " + title + " " + isLocked, localWidth + 28, localHeight + 4,
				0);
		manual.fonts.drawString(toUnlock + " ", localWidth + 50, localHeight + 18, 0);
		manual.fonts.drawString(required + " " + suchAs, localWidth + 70 - (int) (required.length() * 1.8),
				localHeight + 26, 0);
		int yoffset = 0;
