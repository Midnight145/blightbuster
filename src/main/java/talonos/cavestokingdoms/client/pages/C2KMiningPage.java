...
public class C2KMiningPage extends OreDiscoveryPage {
	...
	public void readPageFromXML(Element element) {
		...
		if (nodes != null) { description = nodes.item(0).getTextContent(); }

		nodes = element.getElementsByTagName("min");
		if (nodes != null) { minheight = Integer.parseInt(nodes.item(0).getTextContent()); }
		nodes = element.getElementsByTagName("max");
		if (nodes != null) { maxheight = Integer.parseInt(nodes.item(0).getTextContent()); }

		nodes = element.getElementsByTagName("location");
		if (nodes != null && nodes.item(0) != null && nodes.item(0).getTextContent() != null) { location = nodes.item(0).getTextContent(); }
		...
		if (nodes != null && nodes.item(0) != null) {
			...
			if (iconItem != null) {
				...
				if (requiredLevel == -1) { requiredLevel = 0; }
		...
		if (background == null) { logger.warn(nodes.item(0).getTextContent() + " could not be found in the image cache(location)!"); }
	}
	...
	private void drawNormal(int localWidth, int localHeight) {
		...
		if (this.minesAsLevel != -1) {
			manual.fonts.drawString(
					minesAsLevel + ": " + this.minesAsLevel + " (" + HarvestLevels.getHarvestLevelName(this.minesAsLevel) + ")",
					localWidth + 0, localHeight + 25, 0);
		}
		manual.fonts.drawSplitString(requiredToMine + ": " + requiredLevel + " (" + HarvestLevels.getHarvestLevelName(requiredLevel) + ")",
				localWidth + 0, localHeight + 35, 100, 0);
		manual.fonts.drawSplitString("\u00a7l" + biome + ": \u00a7r" + whereFound, localWidth + 0, localHeight + 60, 106, 0);
		manual.fonts.drawSplitString("\u00a7l" + height + ": \u00a7r" + between + " " + minheight + " " + and + " " + maxheight + ".",
				localWidth + 0, localHeight + 105, 100, 0);
	...
	public void renderBackgroundLayer(int localWidth, int localHeight) {
		if (isDiscovered(requiredLevel)) {
			...
			if (background != null) { manual.getMC().getTextureManager().bindTexture(background); }
