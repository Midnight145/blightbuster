...
public class AltarBlock extends CtKBlock {
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
		...
		if (side == 1) {
			// We are on top.
			if (northCovered) {
				if (eastCovered) { return icons[BOTTOMLEFT]; }
				if (westCovered) { return icons[BOTTOMRIGHT]; }
			}
			if (southCovered) {
				if (eastCovered) { return icons[TOPLEFT]; }
		...
		if (side == 2 || side == 3) {
			if (eastCovered) { return icons[RIGHTSIDE]; }
			if (westCovered) { return icons[LEFTSIDE]; }
		}
		return icons[PLAIN];
	}
}
