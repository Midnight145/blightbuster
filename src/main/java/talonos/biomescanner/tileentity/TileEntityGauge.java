...
public class TileEntityGauge extends TileEntity {
	...
	protected void register() { MapScanner.instance.bus().register(this); }
	...
	private void updateGauge() {
		...
		if (this.worldObj.getBlock(xCoord, yCoord - 2, zCoord) != BSBlock.gaugeMid)
			return;
		if (this.worldObj.getBlock(xCoord, yCoord - 1, zCoord) != BSBlock.gaugeTop)
			return;
		...
		if (completion != 1.0f)
			currentStage = (int) Math.floor(totalStages * completion);
		if (completion != 0 && currentStage == 0)
			currentStage = 1;
