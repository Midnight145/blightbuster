...
public class TileEntityGaugeRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity entity, double x, double y, double z, float par8) {
		...
		if (shouldMixIcon) {
			tickCycle = (float) (entity.getWorldObj().getWorldTime() % 40);
		...
		if (shouldMixIcon) {
			...
			renderEngine.bindTexture(new ResourceLocation("biomescanner", getEmptyIcon(tileEntityGauge.getTargetZone())));
			block.render();
