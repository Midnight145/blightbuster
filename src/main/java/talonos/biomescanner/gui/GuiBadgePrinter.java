...
public class GuiBadgePrinter extends GuiContainer {
	...
	private static final ResourceLocation background = new ResourceLocation("biomescanner", "textures/gui/badgePrinter.png");
	...
	protected void drawGuiContainerForegroundLayer(int p_146979_1_, int p_146979_2_) {
		this.fontRendererObj.drawString(this.printer.hasCustomInventoryName() ? this.printer.getInventoryName()
				: I18n.format(this.printer.getInventoryName(), new Object[0]), 8, 6, 4210752);
		this.fontRendererObj.drawString(this.player.hasCustomInventoryName() ? this.player.getInventoryName()
				: I18n.format(this.player.getInventoryName(), new Object[0]), 34, this.ySize - 96 + 2, 4210752);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
		...
		for (int badgeY = 0; badgeY < 6; badgeY++) {
			for (int badgeX = 0; badgeX < 12; badgeX++) {
				this.drawTexturedModelRectFromIcon(k + 8 + (badgeX * 18), l + 18 + (badgeY * 18), BSItems.badge.getZoneSilhouette(), 16,
						16);
			}
		}
