...
public class ContainerBadgePrinter extends Container {
	...
	@Override
	public boolean canInteractWith(EntityPlayer player) { return tileEntity.isUseableByPlayer(player); }
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
		...
		if (slotIndex < tileEntity.getSizeInventory()) {
			...
			if (!this.mergeItemStack(currentStack, tileEntity.getSizeInventory(), 36 + tileEntity.getSizeInventory(),
					true))
				return null;
