		...
		for (int x = 0; x <= maxBlocks; x++) {
			for (int z = 0; z <= maxBlocks; z++) {
				for (int y = 0; y < 50; y++) {
					...
					Minecraft.getMinecraft().theWorld.spawnParticle(message.doUseFlameParticles() ? "flame" : "smoke",
							x + message.getSpawnX() + r.nextDouble(), height - 5 + (r.nextDouble() * 15),
							z + message.getSpawnZ() + r.nextDouble(), 0, d1, d2);
				}
			}
		}
