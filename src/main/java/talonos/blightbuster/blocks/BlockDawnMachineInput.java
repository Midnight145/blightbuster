...
public class BlockDawnMachineInput extends BlockMultiblock {
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        if (side == 0 || side == 1) return backgroundTop;
    ...
    public Aspect getSpoutAspect(int side, int meta) {
        ...
        switch (inputBlockIndex) {
            case 0:
                ...
                if (sideDir == ForgeDirection.NORTH) return Aspect.PLANT;
                else if (sideDir == ForgeDirection.SOUTH) return Aspect.AURA;
                else return null;
            case 1:
                // East lower block, sano is north, machina is south
                if (sideDir == ForgeDirection.NORTH) return Aspect.HEAL;
                else if (sideDir == ForgeDirection.SOUTH) return Aspect.MECHANISM;
                else return null;
            case 2:
                // West upper block, west is ordo, north is arbor, south is vacuos
                if (sideDir == ForgeDirection.SOUTH) return Aspect.VOID;
                else if (sideDir == ForgeDirection.NORTH) return Aspect.TREE;
                else if (sideDir == ForgeDirection.WEST) return Aspect.ORDER;
                else return null;
            default:
                // East upper block, east is aer, north is ignis, south is cognitio
                if (sideDir == ForgeDirection.NORTH) return Aspect.FIRE;
                else if (sideDir == ForgeDirection.SOUTH) return Aspect.MIND;
                else if (sideDir == ForgeDirection.EAST) return Aspect.AIR;
                else return null;
        }
    }
}
