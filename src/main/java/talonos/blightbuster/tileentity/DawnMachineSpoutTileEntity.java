...
public class DawnMachineSpoutTileEntity extends TileEntity implements IEssentiaTransport, IAspectContainer {
	...
	protected DawnMachineTileEntity getController() {
		TileEntity controller = BBBlock.dawnMachineInput.getMultiblockController(getWorldObj(), this.xCoord,
	...
	public void updateEntity() {
		...
		if ((!this.worldObj.isRemote) && (getWorldObj().getWorldTime() % 5 == 0)) {
			for (int i = 0; i < 6; i++) {
				...
				TileEntity te = ThaumcraftApiHelper.getConnectableTile(this.worldObj, this.xCoord, this.yCoord,
						this.zCoord, dir);
				if (te != null) {
	...
	@Override
	public boolean isConnectable(ForgeDirection forgeDirection) { return getEssentiaType(forgeDirection) != null; }
	
	@Override
	public boolean canInputFrom(ForgeDirection forgeDirection) { return isConnectable(forgeDirection); }
	
	@Override
	public boolean canOutputTo(ForgeDirection forgeDirection) { return false; }
	
	@Override
	public void setSuction(Aspect aspect, int i) {}
	
	@Override
	public Aspect getSuctionType(ForgeDirection forgeDirection) { return getEssentiaType(forgeDirection); }
