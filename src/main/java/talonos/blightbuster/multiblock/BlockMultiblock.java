...
public abstract class BlockMultiblock extends Block {
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        ...
        Pair<MultiblockEntry, Integer> entry = getMultiblock()
            .getEntry(world, x, y, z, getOrientation(meta), block, meta);

        if (entry != null) getMultiblock()
            .convertMultiblockWithOrientationFromSideBlock(world, x, y, z, entry.getValue(), true, entry.getKey());
    ...
    public TileEntity getMultiblockController(World world, int x, int y, int z) {
        ...
        Pair<MultiblockEntry, Integer> entry = getMultiblock()
            .getEntry(world, x, y, z, getOrientation(meta), this, meta);
        ...
        int thisXOffset = xAxis.offsetX * entry.getKey()
            .getXOffset() + zAxis.offsetX
                * entry.getKey()
                    .getZOffset();
        int thisZOffset = xAxis.offsetZ * entry.getKey()
            .getXOffset() + zAxis.offsetZ
                * entry.getKey()
                    .getZOffset();
        int thisYOffset = entry.getKey()
            .getYOffset();
        ...
        return world.getTileEntity(
            x - thisXOffset + controllerXOffset,
            y - thisYOffset + controllerYOffset,
            z - thisZOffset + controllerZOffset);
    }
}
