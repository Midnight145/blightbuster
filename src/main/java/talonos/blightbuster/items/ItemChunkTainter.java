...
public class ItemChunkTainter extends Item {
	...
	public ItemChunkTainter() {
		this.setUnlocalizedName(BlightBuster.MODID + "_" + BBStrings.chunkTainterName);
		GameRegistry.registerItem(this, BBStrings.chunkTainterName);
		this.setCreativeTab(CreativeTabs.tabMaterials);
		this.setTextureName(BlightBuster.MODID + ":" + BBStrings.chunkTainterName);
	}
	...
	public ItemStack onItemRightClick(ItemStack itemStack, World theWorld, EntityPlayer thePlayer) {
		...
		if (!theWorld.isRemote) {
			...
			for (int xLoc = x; xLoc < x + 16; xLoc++) {
				for (int zLoc = z; zLoc < z + 16; zLoc++) {
			...
			thePlayer.addChatMessage(new ChatComponentText("  Chunk should now be tainted.")); // Well it should. :/

			// While we're at it, UBify all ores in the area around us as well.

			UBAPIHook.ubAPIHook.ubOreTexturizer.redoOres(x, z, theWorld);
