...
public class ItemPurityFocus extends ItemFocusBasic {
	...
	public ItemStack onFocusRightClick(ItemStack itemstack, World world, EntityPlayer p, MovingObjectPosition mop) {
		...
		if (pointedEntity != null && !world.isRemote) {
			...
			if (pointedEntity instanceof EntityCustomNpc) {
				EntityCustomNpc npc = (EntityCustomNpc) pointedEntity;
				if (npc.linkedName.equals("TaintedOcelot")) {
					this.convertFromTo(pointedEntity, new EntityOcelot(pointedEntity.worldObj));
					wand.consumeAllVis(itemstack, p, this.getVisCost(itemstack), true, false);
				}
				else if (npc.linkedName.equals("TaintedWolf")) {
					this.convertFromTo(pointedEntity, new EntityWolf(pointedEntity.worldObj));
					wand.consumeAllVis(itemstack, p, this.getVisCost(itemstack), true, false);
				}
				else if (npc.linkedName.equals("TaintedTownsfolk")) {
					this.convertFromTo(pointedEntity, new EntityVillager(pointedEntity.worldObj));
					wand.consumeAllVis(itemstack, p, this.getVisCost(itemstack), true, false);
				}
			}
		}
