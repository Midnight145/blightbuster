...
public class PurityFocusFX {
    ...
    public static void fire(EntityLivingBase entity, EntityPlayer player) {
        ...
        for (int i = (int) count; i >= 0; i--) {
            double w = entity.worldObj.rand.nextGaussian() * entity.width * 0.6;
            double d = entity.worldObj.rand.nextGaussian() * entity.width * 0.6;
            double h = entity.worldObj.rand.nextFloat() * entity.height * 0.05F + entity.height * (i / count);
    ...
    public static void clean(EntityLivingBase entity, EntityPlayer player) {
        ...
        if (!lastCleanTime.containsKey(player.getUniqueID())
            ...
            entity.worldObj.playSound(entity.posX, entity.posY, entity.posZ, "thaumcraft:gore", 0.75F, 1, false);
        ...
        for (int i = 10; i > 0; i--) {
            ...
            entity.worldObj.spawnParticle("witchMagic", entity.posX + w, entity.posY + h, entity.posZ + d, d0, d1, d2);
        }
    }
    ...
    public static void node(EntityPlayer player, int x, int y, int z) {
        ...
        for (int i = 12; i >= 0; i--) {
            float w = (float) (player.worldObj.rand.nextGaussian() + 1) / 2F;
            float d = (float) (player.worldObj.rand.nextGaussian() + 1) / 2F;
            float h = (float) (player.worldObj.rand.nextGaussian() + 1) / 2F;
