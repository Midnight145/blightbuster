...
public class TalonosWandTriggerManager implements IWandTriggerManager {
	...
	public boolean performTrigger(World world, ItemStack wand, EntityPlayer player, int x, int y, int z, int side, int event) {
			...
			if (ResearchManager.isResearchComplete(player.getCommandSenderName(), "DAWNMACHINE")) {
				...
				if (success) {
					...
					if (convertedBlock instanceof BlockMultiblock) {
						...
						if (controller != null) {
							pairDawnMachineToWand(world, wand, controller.getWorldObj().provider.dimensionId, controller.xCoord,
		...
		case 1:
			...
			if (convertedBlock instanceof BlockMultiblock) {
				...
				if (controller != null) {
					...
					if (!isWandPaired(wand, controller.getWorldObj().provider.dimensionId, controller.xCoord, controller.yCoord,
							controller.zCoord)) {
						pairDawnMachineToWand(world, wand, controller.getWorldObj().provider.dimensionId, controller.xCoord,
								controller.yCoord, controller.zCoord);
		...
		case 2:
			...
			for (int clearY = 0; clearY < 5; clearY++) {
				for (int clearX = -2; clearX <= 2; clearX++) {
					for (int clearZ = -2; clearZ <= 2; clearZ++) {
						if (clearY == 0 && clearX == 0 && clearZ == 0) { continue; }
						if (player.getPlayerCoordinates().posX == x + clearX && player.getPlayerCoordinates().posY == y + clearY
			...
			if (result) {
				...
				pairDawnMachineToWand(world, wand, controller.getWorldObj().provider.dimensionId, controller.xCoord, controller.yCoord,
						controller.zCoord);
		...
		case 3:
			...
			if (!ResearchManager.isResearchComplete(player.getCommandSenderName(), "DAWNCHARGER")) { return false; }
	...
	private void moveDawnCharger(World world, ItemStack wand) {
		...
		DawnChargerTileEntity dawnCharger = (DawnChargerTileEntity) world.getTileEntity(chargerTag.getInteger("X"),
				chargerTag.getInteger("Y"), chargerTag.getInteger("Z"));
	...
	private void pairDawnMachineToWand(World world, ItemStack wand, int dimension, int x, int y, int z) {
		...
		DawnChargerTileEntity dawnCharger = (DawnChargerTileEntity) world.getTileEntity(wandChargerTag.getInteger("X"),
				wandChargerTag.getInteger("Y"), wandChargerTag.getInteger("Z"));
	...
	private boolean createDawnMachine(ItemStack stack, EntityPlayer player, World world, int x, int y, int z) {
		...
		if (entry != null) {
			if (wand.consumeAllVisCrafting(stack, player, new AspectList().add(Aspect.ORDER, 20), true)) {
				if (!world.isRemote) {
					BBBlock.dawnMachineMultiblock.convertMultiblockWithOrientationFromSideBlock(world, x, y, z, entry.getValue(), false,
							entry.getKey());
					return true;
				}
				return false;
